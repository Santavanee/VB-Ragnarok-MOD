...
Write-Output "--- Units m1000..m1046 tactics ---"
foreach ($u in $units) {
    if ($u.id -match "^m10[0-4][0-9]$") {
        $validTac = @()
        foreach ($tac in $u.tactics) {
            if ($tac.id -and $tactics.ContainsKey($tac.id)) {
                $validTac += $tactics[$tac.id].name + " (" + $tac.id + ")"
            }
        }
        if ($validTac.Count -gt 0) {
            Write-Output ($u.name + " [" + $u.id + "]: " + ($validTac -join ", "))
        }
    }
}
