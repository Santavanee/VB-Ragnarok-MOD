...
$u = $units | Where-Object { $_.id -eq 'm0655' }
Write-Output ("Hel m0655: trick count=" + $u.trick.Count + ", tactics count=" + $u.tactics.Count)
Write-Output ("Trick: " + $passives[$u.trick[0].id] + " (" + $u.trick[0].id + ") [" + $u.trick[0].power + "]")
foreach ($t in $u.tactics) {
    $vt = $tactics[$t.id]
    Write-Output ("  " + $t.id + " | " + $vt.name + " (Lv." + $vt.level + " Cost:" + $vt.cost + "): " + $vt.comment)
}
