...
Write-Output "=== Units matching Pirate / Sea / Sailor / Ship / Corsair / Water ==="
foreach ($u in $units) {
    if ($u.name -match "Pirate|Sailor|Corsair|Sea|Ship|Aqua|Ocean|Viking|Captain|Mary" -or $u.comment -match "Pirate|Sailor|Corsair|Sea|Ship|Ocean|Viking|Captain") {
        $tricks = @()
        foreach ($tr in $u.trick) {
            $pName = if ($passives.ContainsKey($tr.id)) { $passives[$tr.id] } else { $tr.id }
            $tricks += ($pName + " (" + $tr.id + ") [" + $tr.power + "]")
        }
        Write-Output ($u.id + " : " + $u.name + " (" + $u.tribe + ") [tactics: " + $u.tactics.Count + ", trick: " + ($tricks -join ", ") + "]")
