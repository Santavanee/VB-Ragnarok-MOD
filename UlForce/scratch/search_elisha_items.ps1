...
$items = Read-BytesFile "ItemDataSet.bytes"
foreach ($it in $items) {
    if ($it.name -match "Caduceus" -or $it.name -match "Admiral" -or $it.name -match "Coat") {
        Write-Output ("Item: " + $it.id + " | " + $it.name + " | equipType=" + $it.equipType)
    }
}
