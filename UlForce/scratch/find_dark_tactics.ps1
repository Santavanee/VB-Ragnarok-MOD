...
foreach ($u in $units) {
    if ($u.tactics.Count -ge 3 -and $u.id -notmatch '^m0[0-5]') {
        $found = $false
        foreach ($tac in $u.tactics) {
            $vt = $tactics[$tac.id]
            if ($vt -and ($vt.name -match "Raise|Underworld|Necro|Abyss|Dark|Death|Miasma|Eclipse|Shadow" -or $vt.comment -match "Revive.*Dark|Undead|Poison")) {
                $found = $true; break
            }
        }
        if ($found) {
            Write-Output ($u.id + " : " + $u.name + " (" + $u.tribe + ")")
            foreach ($tac in $u.tactics) {
                $vt = $tactics[$tac.id]
                if ($vt) {
